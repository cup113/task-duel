...
def run_pocket_base():
    all_wait(
        general_popen(
            ...
            "pocketbase-typegen",
            "--db",
            ROOT / "server" / "db" / "pb_data" / "data.db",
            "--out",
            ROOT / "server" / "types" / "pocketbase-types.d.ts",
        )
    )
    return general_popen(
        ROOT / "server" / "db" / "pocketbase.exe", "serve", "--http", "127.0.0.1:4137"
    )
...
def run_vite():
    return general_popen(pnpm, "run", "dev", cwd=ROOT)
    ...
    return (
        ...
        general_popen(
            ...
            env_add={
                ...
                "POCKETBASE_URL": "http://localhost:4137/",
            },
        ),
        run_vite(),
    )
